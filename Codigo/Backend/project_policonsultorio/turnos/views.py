...
class RegistroTurnoView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        #Validamos que la fecha pedida no sea anterior a la actual
        fecha_actual = datetime.now().date()
        if fecha < fecha_actual:
            return Response({"error": "No se puede agendar un turno en una fecha anterior a la actual."}, status=400)
        ...
        #Validamos que no esté ocupado el horario seleccionado
